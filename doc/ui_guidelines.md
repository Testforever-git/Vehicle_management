...
- 所有list可以选择一页显示的记录数量。 选择放在右上角，固定为20/50。 记录数量超过选择的数量时分页

## 11 年份显示规定
- 年份显示分西历和和历。
  a.年份只需要显示一种 要么西历，要么和历，不需要同时显示。
  b.所有需要显示年份的控件，如果是textbox 就在右方放置一个checkbox'和历'，勾选则为和历，不勾选则为西历
    如果是表格(list)，则在表格上方放置一个checkbox'和历'，勾选则list中的年份显示都为和历，不勾选则为西历
  c.年份转换不通过DB，DB中只保存西历(如2025)，通过app\static\i18n\convert_year.yaml来进行转换。
  格式为:
  1988年:昭和63年
  1989年:昭和64年
  1990年:平成2年
