...
@bp.post("/portal/rentals/<int:vehicle_id>/apply", endpoint="portal_rental_apply_post_v2")
def portal_rental_apply_post(vehicle_id: int):
    customer = get_current_customer()
    if not customer.is_authenticated:
    ...
    def _tier_fee(distance_km):
        ...
        for tier in tiers:
            ...
            if distance_km >= float(tier["min_km"]) and (max_km is None or distance_km <= float(max_km)):
                ...
                if action == "fixed_fee":
                    return int(tier.get("fee") or 0), action
                return 0, action
        return 0, None
    ...
    pickup_fee = 0
    dropoff_fee = 0
    pickup_fee_action = None
    dropoff_fee_action = None
    ...
    if pickup_method == "address":
        ...
        pickup_fee, pickup_fee_action = _tier_fee(pickup_distance)
    ...
    if dropoff_method == "address":
        ...
        dropoff_fee, dropoff_fee_action = _tier_fee(dropoff_distance)
        ...
        if dropoff_store and vehicle_store and dropoff_store.get("id") != vehicle_store.get("id"):
            ...
            tier_fee, dropoff_fee_action = _tier_fee(store_distance)
            dropoff_fee = int(tier_fee * 0.5)
    ...
    price_snapshot = {
        ...
        "pickup_fee_action": pickup_fee_action,
        "dropoff_fee_action": dropoff_fee_action,
    ...
    return redirect(url_for("portal.portal_rental_booking_magic_view", access_token=access_token, lang=request.args.get("lang")))


@bp.get("/portal/rentals/booking/<access_token>", endpoint="portal_rental_booking_magic_view")
...
@bp.post("/portal/rentals/<int:vehicle_id>/apply", endpoint="portal_rental_apply_post")
def portal_rental_apply_post(vehicle_id: int):
    customer = get_current_customer()
    if not customer.is_authenticated:
        next_url = url_for("portal.portal_rental_detail", vehicle_id=vehicle_id, lang=request.args.get("lang"))
...
@bp.post("/portal/rentals/<int:vehicle_id>/apply", endpoint="portal_rental_apply")
def portal_rental_apply(vehicle_id: int):
    customer = get_current_customer()
    if not customer.is_authenticated:
        next_url = url_for("portal.portal_rental_detail", vehicle_id=vehicle_id, lang=request.args.get("lang"))
        return redirect(url_for("portal.portal_customer_login", next_url=next_url, lang=request.args.get("lang")))
