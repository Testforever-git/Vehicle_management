...
PHOTO_CATEGORY = "vehicle_photo"
LEGACY_PHOTO_CATEGORY = "Vehicle_photo"
...
def _vehicle_image_dirs(vin: str):
    ...
    photo_dir = os.path.join(_image_base_dir(), safe_vin, PHOTO_CATEGORY)
    legacy_photo_dir = os.path.join(_image_base_dir(), safe_vin, LEGACY_PHOTO_CATEGORY)
...
def vehicle_image(vin: str, category: str, filename: str):
    ...
    if category not in {"legal_doc", PHOTO_CATEGORY, LEGACY_PHOTO_CATEGORY}:
        abort(404)
    safe_vin = _safe_vin(vin)
    base_dir = _image_base_dir()
    if category == "legal_doc":
        ...
        candidate_dirs = [
            os.path.join(base_dir, safe_vin, PHOTO_CATEGORY),
            os.path.join(base_dir, safe_vin, LEGACY_PHOTO_CATEGORY),
        ]
...
def vehicle_edit(vehicle_id: int):
    ...
    if request.method == "POST":
        ...
        delete_vehicle_media(
            vehicle_id,
            "vehicle_photo",
            _media_rel_paths(vin, PHOTO_CATEGORY, removed_photos)
            + _media_rel_paths(vin, LEGACY_PHOTO_CATEGORY, removed_photos),
        )
        ...
        create_vehicle_media(
            vehicle_id,
            "vehicle_photo",
            _media_rel_paths(vin, PHOTO_CATEGORY, new_photos),
            get_current_user().user_id,
        )
