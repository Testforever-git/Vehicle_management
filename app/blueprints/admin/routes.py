...
from ...repositories.feature_catalog_repo import list_feature_catalog, upsert_feature_catalog
...
@bp.route("/admin/feature-catalog", methods=["GET", "POST"])
def feature_catalog():
    if not _require_admin():
        return redirect(url_for("ui.dashboard"))
    ...
    if request.method == "POST":
        ...
        value_type = (request.form.get("value_type") or "bool").strip()
        enum_options_json = (request.form.get("enum_options_json") or "").strip()
        category_code = (request.form.get("category_code") or "").strip() or None
        is_active = request.form.get("is_active") == "1"
        enum_options = None
        if enum_options_json:
            try:
                enum_options = json.loads(enum_options_json)
            except json.JSONDecodeError:
                enum_options = None
        if code and name_jp and name_cn:
            upsert_feature_catalog(
                code=code,
                name_jp=name_jp,
                name_cn=name_cn,
        ...
        return redirect(url_for("admin.feature_catalog", lang=lang))
